 ...
 * This includes the substitute and the execution date since this is the same for approversL1 and approversL2.
 * </p>
 * <p> This is a customization for <b><code>Banco do Brasil Project</code></b> </p>
 * <p>
 * It has to be specified if operator and phase parameters should be set.
 ...
 */
public class IssueSetOperatorCommandBB extends SetOperatorCommand {
	...
	public CommandResult execute(CommandContext cc) throws BLException {
		...
		if (isCreationPhase) {
			if (isMgmtApproverL1) {
				fillAttributes(cc, mgmtApproverL1Attributes);
			} else if (isMgmtApproverL2) {
				fillAttributes(cc, mgmtApproverL2Attributes);
			} else if (isImplApproverL1) {
				fillAttributes(cc, implApproverL1Attributes);
			} else if (isImplApproverL2) {
				fillAttributes(cc, implApproverL2Attributes);
			} else if (isTechApproverL1) {
				fillAttributes(cc, techApproverL1Attributes);
			} else if (isTechApproverL2) {
				fillAttributes(cc, techApproverL2Attributes);
			} else {
				throw new IllegalArgumentException();
			}
		} else if (isExecutionPhase) {
			if(isOwner) {
				fillAttributes(cc, ownerAttributes);
			} else if(isReviewer) {
				fillAttributes(cc, reviewerAttributes);
			} else if (isMgmtApproverL1) {
				fillAttributes(cc, execMgmtApprL1Attributes);
			} else if (isMgmtApproverL2) {
				fillAttributes(cc, execMgmtApprL2Attributes);
			} else if (isImplApproverL1) {
				fillAttributes(cc, execImplApprL1Attributes);
			} else if (isImplApproverL2) {
				fillAttributes(cc, execImplApprL2Attributes);
			} else {
				throw new IllegalArgumentException();
			}
		} else {
	...
	@Override
	public void fillAttributes(CommandContext cc, String[] p_attributes) throws BLException {
		final boolean isIssueManager = isIssueManager(cc.getChainContext().getUserContext());
		
		if (isIssueManager) {
			fillAttributesForIssueManagers(cc, p_attributes);
		} else {
			super.fillAttributes(cc, p_attributes);
		} 
	}
	
	/**
	 * Specific methods to meet Issue Administrators be able to do anything during the ISSUE's workflow execution.
	 * 
	 * @param cc
	 * @param p_attributes
	 * @throws BLException
	 */
	protected void fillAttributesForIssueManagers(CommandContext cc, String[] p_attributes) throws BLException{
		IAppObj appObj = cc.getChainContext().getTriggeringAppObj();
		IUserContext userContext = cc.getChainContext().getUserContext();
		IUserContext fullGrantCtx = ContextFactory.createFullGrantUserContext(userContext);

		IListAttribute performedBy = (IListAttribute) appObj.getAttribute(appObj.getAttributeType(p_attributes[1]));
		IListAttribute substitute = (IListAttribute) appObj.getAttribute(appObj.getAttributeType(p_attributes[2]));
		IDateAttribute performedDate = (IDateAttribute) appObj.getAttribute(appObj.getAttributeType(p_attributes[3]));
		
		Date now = DateUtils.today();
		performedDate.setRawValue(now);
		performedBy.removeAllElements(fullGrantCtx);
		performedBy.addLastElement(userContext.getUser(), fullGrantCtx);
		substitute.removeAllElements(fullGrantCtx);
		IUserAppObj substUser = userContext.getSubstitudeUser();
		if (null != substUser) {
			substitute.addLastElement(substUser, fullGrantCtx);
		}
	}
	
	protected boolean isIssueManager(IUserContext userContext){
		List<IOVID> issueManagerGroups = userContext.getUserRelations().getGroupsIDs(Enumerations.USERROLE_TYPE.ISSUEMANAGER, RoleLevel.C);
		return !issueManagerGroups.isEmpty();
	}
